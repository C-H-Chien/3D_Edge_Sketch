...
namespace GetReprojectedEdgel {
    ...
    Eigen::MatrixXd get_Reprojected_Edgel::getGamma3Pos(Eigen::MatrixXd pt_edge_HYPO1, Eigen::MatrixXd edgels_HYPO2, std::vector<Eigen::Matrix3d> All_R, std::vector<Eigen::Vector3d> All_T, int VALID_INDX, Eigen::Matrix3d K) {
    ...
    Eigen::MatrixXd get_Reprojected_Edgel::getGamma3Tgt(Eigen::MatrixXd pt_edge_HYPO1, Eigen::MatrixXd edgels_HYPO2, std::vector<Eigen::Matrix3d> All_R, std::vector<Eigen::Vector3d> All_T, int VALID_INDX, Eigen::Matrix3d K) {
        MultiviewGeometryUtil::multiview_geometry_util util;
        Eigen::Vector3d pt_edgel_HYPO1;
        pt_edgel_HYPO1 << pt_edge_HYPO1(0,0), pt_edge_HYPO1(0,1), 1;
        Eigen::Vector3d Gamma1 = K.inverse() * pt_edgel_HYPO1;
        ...
        Eigen::Matrix3d R31 = util.getRelativePose_R21(R1, R3);
        Eigen::Vector3d T31 = util.getRelativePose_T21(R1, R3, T1, T3);
        Eigen::Vector3d tgt1_meters = getTGT_Meters(pt_edge_HYPO1, K);
        ...
        for (int idx_HYPO2 = 0; idx_HYPO2 < edgels_HYPO2.rows(); idx_HYPO2++){
            ...
            double rho3 = rho1 * double(e3.transpose() * R31 * Gamma1) + double(e3.transpose()*T31);
            Eigen::Vector3d Gamma3 = 1 / rho3 * (R31 * rho1 * Gamma1 + T31);
            Eigen::Vector3d tgt2_meters = getTGT_Meters(pt_edge_HYPO2, K);
