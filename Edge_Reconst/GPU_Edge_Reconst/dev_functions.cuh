...
{
  /*
  numerator   = (e1'*T12 - (e3'*T12)*(e1'*gamma2))*R13*gamma1 + ((e3'*R12*gamma1)*(e1'*gamma2) - (e1'*R12*gamma1))*T13;
denominator = (e3'*R13*gamma1)*(e1'*T12-(e3'*T12)*(e1'*gamma2)) + ((e3'*R12*gamma1)*(e1'*gamma2)-(e1'*R12*gamma1))*e3'*T13;
...
point3 = K * gamma3;      
  */
  ...
  n_e1 = s_R21(0,0)*n_e1 + s_R21(1,0)*n_e2 + s_R21(2,0)*n_e3;
  n_e2 = s_R21(0,1)*n_e1 + s_R21(1,1)*n_e2 + s_R21(2,1)*n_e3;
  n_e3 = s_R21(0,2)*n_e1 + s_R21(1,2)*n_e2 + s_R21(2,2)*n_e3;

  //> Now, reuse the memory from r_mapped_tangent3 to compute the cross product of s_N1_vec and [n_e1; n_e2; n_e3]
  //T Tangent_v1[3];
  r_mapped_tangent3[0] = s_N1_vec[1]*n_e3 - s_N1_vec[2]*n_e2; //> This is x of T_v1
  r_mapped_tangent3[1] = s_N1_vec[2]*n_e1 - s_N1_vec[0]*n_e3; //> This is y of T_v1
  r_mapped_tangent3[2] = s_N1_vec[0]*n_e2 - s_N1_vec[1]*n_e1; //> This is z of T_v1

  //> We reuse the memory of from n_e1, n_e2, and n_e3 again for computing Tangent_v3
  n_e1 = s_R31(0,0)*r_mapped_tangent3[0] + s_R31(0,1)*r_mapped_tangent3[1] + s_R31(0,2)*r_mapped_tangent3[2]; //> This is x of T_v3
  n_e2 = s_R31(1,0)*r_mapped_tangent3[0] + s_R31(1,1)*r_mapped_tangent3[1] + s_R31(1,2)*r_mapped_tangent3[2]; //> This is y of T_v3
  n_e3 = s_R31(2,0)*r_mapped_tangent3[0] + s_R31(2,1)*r_mapped_tangent3[1] + s_R31(2,2)*r_mapped_tangent3[2]; //> This is z of T_v3

  //> Here we reuse the memory r_mapped_tangent3 for computing the projected tangent on the VALID view
  r_mapped_tangent3[0] = n_e1 - n_e3*r_mapped_gamma3_pixel[0]; //> x of projected t3
  r_mapped_tangent3[1] = n_e2 - n_e3*r_mapped_gamma3_pixel[1]; //> y of projected t3
