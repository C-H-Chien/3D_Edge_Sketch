...
#ifndef vgl_polygon_HPP
#define vgl_polygon_HPP
//:
// \file
...
#include "./vgl_point_2d_CH.hpp" // needed for std::vector instantiations
...
class vgl_polygon_CH
{
 public:
  ...
  //vgl_polygon_CH() = default;

  //: Construct an empty polygon, setting the number of (empty) sheets
  explicit vgl_polygon_CH(unsigned int nr_sheets) : sheets_(nr_sheets) {}

  //: Add a new point to the last sheet
  void push_back_(T x, T y) {
