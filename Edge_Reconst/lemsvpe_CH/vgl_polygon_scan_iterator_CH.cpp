// This is core/vgl/vgl_polygon_scan_iterator_CH.hxx
#ifndef vgl_polygon_scan_iterator_CH_cpp_
#define vgl_polygon_scan_iterator_CH_cpp_
//:
// \file
...
#include <iostream>
#include <algorithm>
#include "./vgl_polygon_scan_iterator_CH.hpp"
//#ifdef _MSC_VER
//#  include <vcl_msvc_warnings.h>
//#endif

// It used to be necessary to add 0.5 to the scanline coordinates
// obtained from a vgl_polygon_scan_iterator_CH. Presumably this had
// something to do with pixels and rendering them, but that issue is
// irrelevant to a polygon_scan_iterator.
//
// I think it is clear what a vgl_polygon_scan_iterator_CH should do: tell
...
{
  compare_vertind(typename vgl_polygon_CH<T>::sheet_t* chs) : chs_(chs)
  {}

  inline bool operator()(const typename vgl_polygon_scan_iterator_CH<T>::vertind &u,
                         const typename vgl_polygon_scan_iterator_CH<T>::vertind &v) const
  {
    return chs_[u.chainnum][u.vertnum].y() < chs_[v.chainnum][v.vertnum].y();
  }

  typename vgl_polygon_CH<T>::sheet_t* chs_;
};

template <class T>
struct compare_crossedges
{
  inline bool operator()(const typename vgl_polygon_scan_iterator_CH<T>::crossedge &u,
                         const typename vgl_polygon_scan_iterator_CH<T>::crossedge &v) const
  {
    return u.x < v.x;
  }
};
...
template <class T>
inline static void local_qsort(typename vgl_polygon_scan_iterator_CH<T>::vertind* yverts, int numverts, vgl_polygon_CH<T>& p)
{
  std::sort(yverts, yverts + numverts, compare_vertind<T>(&p[0]));
}

template <class T>
inline static void local_qsort(typename vgl_polygon_scan_iterator_CH<T>::crossedge* crossedges, int numcrossedges)
{
...
//===============================================================
template <class T>
vgl_polygon_scan_iterator_CH<T>::~vgl_polygon_scan_iterator_CH()
{
  delete [] crossedges;
  delete [] yverts;
}
...
//===============================================================
template <class T>
vgl_polygon_scan_iterator_CH<T>::vgl_polygon_scan_iterator_CH(vgl_polygon_CH<T> const& face,
                                                        bool boundaryp):
  poly_(face)
{
  boundp = boundaryp;
...
//===============================================================
/*template <class T>
vgl_polygon_scan_iterator_CH<T>::vgl_polygon_scan_iterator_CH(vgl_polygon_CH<T> const& face, bool boundaryp, vgl_box_2d<T> const& window):
  poly_(face)
{
  boundp = boundaryp;
  have_window = true;
...
//===============================================================
template <class T>
void vgl_polygon_scan_iterator_CH<T>::init()
{
...
//===============================================================
template <class T>
void vgl_polygon_scan_iterator_CH<T>::delete_edge( vertind v )
{
    int j;
...
//===============================================================
template <class T>
void vgl_polygon_scan_iterator_CH<T>::insert_edge( vertind v )
{
  vertind nextvert;
  T dx;
  Point2 p, q;
...
//===============================================================
template <class T>
void vgl_polygon_scan_iterator_CH<T>::reset()
{
...
template <class T>
void vgl_polygon_scan_iterator_CH<T>::get_crossedge_vertices(int * &chainnum, int * &vertnum, int & num_crossedges)
{
...
//===============================================================
template <class T>
bool vgl_polygon_scan_iterator_CH<T>::next( )
{
  do {
...
template <class T>
void vgl_polygon_scan_iterator_CH<T>::get_next_vert( vertind v, vertind & nextvert )
{
        nextvert = v;
        nextvert.vertnum += 1;
...
//  Compiler error says the default return type is int???
template <class T>
void vgl_polygon_scan_iterator_CH<T>::get_prev_vert( vertind v, vertind & prevvert )
{
        prevvert = v;
...
//===============================================================
template <class T>
void vgl_polygon_scan_iterator_CH<T>::display_chains()
{
...
//===============================================================
template <class T>
void vgl_polygon_scan_iterator_CH<T>::display_crossedges()
{
    std::cout << "----- CROSSEDGES -----\n"
...
template class vgl_polygon_scan_iterator_CH<float>;
template class vgl_polygon_scan_iterator_CH<double>;

#undef VGL_POLYGON_SCAN_ITERATOR_INSTANTIATE
#define VGL_POLYGON_SCAN_ITERATOR_INSTANTIATE(T) \
template class vgl_polygon_scan_iterator_CH<T >
