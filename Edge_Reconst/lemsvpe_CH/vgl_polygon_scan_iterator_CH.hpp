...
#ifndef vgl_polygon_scan_iterator_CH_hpp
#define vgl_polygon_scan_iterator_CH_hpp
//:
// \file
...
#include "./vgl_region_scan_iterator_CH.hpp"
#include "./vgl_polygon_CH.hpp"
...
template <class T>
class vgl_polygon_scan_iterator_CH : public vgl_region_scan_iterator_CH
{
  ...
  vgl_polygon_CH<T> poly_; //!< the polygon

 public:
  // Stores coordinates of a 2d point
  typedef typename vgl_polygon_CH<T>::point_t Point2;

  // Constructors/Destructor---------------------------------------------------

  //: Construct with a polygon and bool indicating whether boundary included
  vgl_polygon_scan_iterator_CH(vgl_polygon_CH<T> const& face, bool boundaryp = true);

  //> Edited by CH
  /*vgl_polygon_scan_iterator_CH(vgl_polygon_CH<T> const& face, bool boundaryp): poly_(face)
  {
    boundp = boundaryp;
    have_window = false;
    init();
  ...
  //: Destructor
  ~vgl_polygon_scan_iterator_CH() override;
...
// Internals ---------------------------------------------------------------
 ...
 private:
  ...
  vgl_polygon_scan_iterator_CH(const vgl_polygon_scan_iterator_CH& ) = delete;
  vgl_polygon_scan_iterator_CH& operator= (const vgl_polygon_scan_iterator_CH& ) = delete;
