function [TO_edge_map, gen_edge_map, grad_mag, subpix_id_map] = third_order_edge_detector(img, sigma, n, threshold, QaD_flag)
...
% write img to file for debugging
% dst_path = '/home/chchien/BrownU/research/Differential-Geometry-Edge-Detection/Differential_Geometry_in_Edge_Detection/';
% fullOutputFileName = fullfile(dst_path, 'img_data.txt');
% img_wr = fopen(fullOutputFileName, 'w');
% for i = 1:size(img, 1)
%     for j = 1:size(img, 2)
%         fprintf(img_wr, string(img(i,j)));
%         fprintf(img_wr, '\t');
%     end
%     fprintf(img_wr, '\n');
% end
...
[subpix_x, subpix_y, subpix_dir_x, subpix_dir_y, subpix_grad_x, subpix_grad_y, subpix_id_map] = NMS_token(Ix, Iy, grad_mag, grad_mag>threshold, margin);
...
if (QaD_flag)
    ...
    % -- 1) compute higher-order interpolated derivative maps --
...
% store in another data structure
subpix_x_map = zeros(size(subpix_id_map, 1), size(subpix_id_map, 2));
subpix_y_map = zeros(size(subpix_id_map, 1), size(subpix_id_map, 2));
for i = 1:size(subpix_id_map, 2)
    for j = 1:size(subpix_id_map, 1)
        if (subpix_id_map(j,i) ~= 0)
            id = subpix_id_map(j,i);
            subpix_x_map(j, i) = subpix_x(id);
            subpix_y_map(j, i) = subpix_y(id);
        end
    end
end
